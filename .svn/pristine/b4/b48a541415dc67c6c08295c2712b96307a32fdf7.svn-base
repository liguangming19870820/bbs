...
<div class="w ofc mt">
	<div class="l">
		<div class="showPro">
			<div class="big"><a id="showImg" class="cloud-zoom" href="${path }${img.url }" rel="adjustX:10,adjustY:-1"><img alt="" src="${path }${img.url }"></a></div>
		</div>
	</div>
	<div class="r" style="width: 640px">
		<ul class="uls form">
			<li><h2>依琦莲2014瑜伽服套装新款 瑜珈健身服三件套 广场舞蹈服装 性价比最高的瑜伽服 三件套 送胸垫 支持货到付款</h2></li>
			<li><label>巴  巴 价：</label><span class="word"><b class="f14 red mr">￥128.00</b>(市场价:<del id="delid">￥150.00</del>)</span></li>
			<li><label>商品评价：</label><span class="word"><span class="val_no val3d4" title="4分">4分</span><var class="blue">(已有888人评价)</var></span></li>
			<li><label>运　　费：</label><span class="word" id="yunfei">10元</span></li>
				...
				<div id="colors" class="pre spec">
					...
					<c:forEach items="${colorList }" var="color" varStatus="status">
						<c:if test="${status.first }">
							<a onclick="colorToRed(this,${color.id})" id="c_${color.id }" href="javascript:void(0)" title="${color.name }" class="changToRed"><img width="25" height="25" data-img="1" src="/res/img/pic/ppp00.jpg" alt="${color.name }"><i>${color.name }</i></a>
							<script type="text/javascript">
								$("#selectedColor").val('${color.id}');
							</script>
						</c:if>
						<c:if test="${status.first == false }">
							<a onclick="colorToRed(this,${color.id})" id="c_${color.id }" href="javascript:void(0)" title="${color.name }" class="changToWhite"><img width="25" height="25" data-img="1" src="/res/img/pic/ppp00.jpg" alt="${color.name }"><i>${color.name }</i></a>
						</c:if>
					</c:forEach>
				</div>
			</li>
...
function colorToRed(obj,colorId) {
	$(".changToRed").removeClass().addClass("changToWhite");
	$("#c_"+colorId).removeClass().addClass("changToRed");
	
	$("#sizes a").removeClass().addClass("not-allow");
	var flag = 0;
	<c:forEach items="${skuList }" var="sku">
		if ('${sku.colorId}' == colorId) {
			if (flag == 0) {
				$("#${sku.size}").removeClass().addClass("changToRed");
				flag = 1;
				$(".mr").html("￥${sku.skuPrice}");
				$("#delid").html("${sku.marketPrice}");
				$("#yunfei").html("${sku.deliveFee}元");
				$("#stockInventory").html("${sku.stockInventory}");
				storeCout = "${sku.stockInventory}";
				buyLimit = "${sku.skuUpperLimit}";
				cId = colorId;
				skuId = '${sku.id}';
			} else {
				$("#${sku.size}").removeClass().addClass("changToWhite");
			}
		}
	</c:forEach>
}
function sizeToRed(obj, sizeId) {
	if ($("#"+sizeId).hasClass("not-allow")) {
		return;
	}
	$("#sizes a[class='changToRed']").removeClass().addClass("changToWhite");
	$("#"+sizeId).removeClass().addClass("changToRed");
	<c:forEach items="${skuList }" var="sku">
		if ('${sku.colorId}' == cId && '${sku.size}' == sizeId) {
			$("#${sku.size}").removeClass().addClass("changToRed");
			$(".mr").html("￥${sku.skuPrice}");
			$("#delid").html("${sku.marketPrice}");
			$("#yunfei").html("${sku.deliveFee}元");
...
function addCart(){
	...
	window.location.href='http://localhost:80/cart/add.shtml?skuId='+skuId+'&buyCount='+$("#num").val();
}
//立即购买
function buy(){
